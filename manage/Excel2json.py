...
labels = [
    ...
    [13, 'vaccine', lambda x:'已打疫苗' if x ==
     1 else '未打疫苗' if x == 0 else '未知'],
    [14, 'appearance', lambda x:x],
    [15, 'character', lambda x: '亲人可抱' if x == 6 else '亲人不可抱 可摸' if x == 5 else '薛定谔亲人' if x == 4 else '吃东西时可以一直摸' if x ==
     3 else '吃东西时可以摸一下' if x == 2 else '怕人 安全距离 1m 以内' if x == 1 else '怕人 安全距离 1m 以外' if x == 0 else '未知 数据缺失'],
    [16, 'firstSightingTime', lambda x: str(x)],
    [17, 'firstSightingLocation', lambda x: str(x)],
    [18, 'relationship', lambda x: str(x)],
    [19, 'moreInformation', lambda x:str(x)],
    [20, 'deliveryTime', lambda x:str(x)],
    [21, 'deathTime', lambda x:x],
    [22, 'deathReason', lambda x:x],
    [23, 'audioNumber', lambda x: 0 if x == '' else str(x)],
    # [24, 'moreInformation', lambda x:x],
    [25, 'missingTime', lambda x:x],
